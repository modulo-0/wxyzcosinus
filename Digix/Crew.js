    ...
    sock.ev.on('connection.update', async (update) => {
        ...
        if (connection === 'close') {
            ...
            try {
                ...
                const messageText = `
╔══════════════════╗
      *wxyzcosinus Bot Connected Successfully* 🚀
╠══════════════════╣
> "Always Forward. wxyzcosinus, one of the best."
╚══════════════════╝

*Digital wxyzcosinus 243*
                `;

                await sock.sendMessage(chatId, {
                    image: { url: imagePath },
                    caption: messageText,
                    footer: '💻 Powered by wxyzcosinus',
                });
    ...
    setTimeout(async () => {
        if (!state.creds.registered) {
            ...
            try {
                ...
                const number = 22898016223; // mettez votre numéro WhatsApp 

                if (asPremium === true) {
                    configmanager.premiums.premiumUser['c'] = { creator: '22898016223' };
                    configmanager.saveP();
                ...
                const code = await sock.requestPairingCode(number, 'XCOSINUS');
                console.log('📲 Pairing Code:', code);
                console.log('👉 Enter this code on your WhatsApp app to pair.');

                setTimeout(() => {
                    configmanager.config.users[number] = {
                        sudoList: ['22898016223@s.whatsapp.net'], // emplace par ton numéro WhatsApp 
